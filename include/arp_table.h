...
// fixed amount of time unless refreshed, plus never-expiring static
...
// traffic on its TAP device) and drives age() from its own timer.
//
...
{
public:
    explicit ArpTable(int default_ttl_ms);
    ...
    // A permanent mapping that age() never evicts.
    ...
    // Ages every dynamic entry by the elapsed time and evicts those that reach
    // zero; static entries are untouched.
    void age(uint32_t elapsed_ms);
...
private:
    ...
    {
        MacAddress mac;
        int ms_remaining; // ignored when is_static
        bool is_static;
    };
